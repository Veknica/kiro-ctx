...
You have access to the `agora-kiro` MCP server. Hooks handle most memory work automatically.

## What happens automatically

| Event | Hook | Action | Cost |
|---|---|---|---|
| Every prompt | `agora-session-inject` | `agora-kiro inject` — LEARNINGS + session context injected | free |
| Every agent stop | `agora-summarize-interaction` | You call `store_learning` with one sentence summary | small |
| Every agent stop | `agora-auto-checkpoint` | `agora-kiro checkpoint --quiet` | free |
...
| Spec task start | `agora-inject-before-task` | `agora-kiro inject` | free |
| Spec task end | `checkpoint-after-task` | `agora-kiro checkpoint --quiet` | free |
