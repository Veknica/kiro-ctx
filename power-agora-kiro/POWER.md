...
### Step 1: Install agora-kiro

```bash
pip install git+https://github.com/thebnbrkr/agora-kiro.git
```
...
Create the following files in `.kiro/hooks/`. Replace `agora-kiro` with the full path from Step 1 if it's not on your PATH.
...
```json
{
  ...
  "then": {
    "type": "runCommand",
    "command": "agora-kiro inject --quiet 2>/dev/null || true",
    "timeout": 10
  }
}
```
...
```json
{
  ...
  "then": {
    "type": "runCommand",
    "command": "agora-kiro checkpoint --quiet 2>/dev/null || true",
    "timeout": 10
  }
}
```
...
```json
{
  ...
  "then": {
    "type": "askAgent",
    "prompt": "Before reading this file, call summarize_file from agora-kiro to get the AST outline with function names and line numbers. Then use read_file_range to read only the section relevant to the current task."
  }
}
```
...
```json
{
  ...
  "then": {
    "type": "askAgent",
    "prompt": "Call index_file from agora-kiro for the file that was just written so its symbols are searchable in future sessions."
  }
}
```
...
4. **Editable install** — run `pip install -e /path/to/agora-kiro` to use local changes immediately without reinstalling
5. **Check what's stored** — run `agora-kiro memory` in terminal to see all sessions, learnings, and indexed symbols

---

**Package:** `agora-kiro` (pip)
**Source:** [github.com/thebnbrkr/agora-kiro](https://github.com/thebnbrkr/agora-kiro)
**License:** Apache-2.0
**Install:** `pip install git+https://github.com/thebnbrkr/agora-kiro.git`
