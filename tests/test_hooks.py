...
from agora_kiro.cli import _install_claude_code_hooks  # imported at collection time (CWD = repo root)
...
class TestClaudeHooks:
    ...
    def test_pre_compact_uses_checkpoint_not_state_save(self):
        ...
        assert any("checkpoint" in c for c in commands), (
            "PreCompact should call 'agora-kiro checkpoint'"
        )
...
class TestGeminiHooks:
    ...
    def test_pre_compress_uses_checkpoint(self):
        ...
        commands = _extract_commands(hooks)
        assert any("checkpoint" in c for c in commands), (
            "PreCompress should call 'agora-kiro checkpoint'"
        )
        for cmd in commands:
            assert "state save" not in cmd
...
def _run_cli(*args, env_override=None) -> subprocess.CompletedProcess:
    """Run an agora-kiro CLI command and return the result."""
    import os
    env = os.environ.copy()
    # Use a temp DB so tests don't touch the real ~/.agora-kiro/memory.db
    import tempfile
    env["AGORA_KIRO_DB"] = tempfile.mktemp(suffix=".db")
    if env_override:
        env.update(env_override)
    return subprocess.run(
        [sys.executable, "-m", "agora_kiro.cli"] + list(args),
...
def test_inject_quiet_outputs_context_when_session_exists(tmp_path):
    ...
    stdout like: CONTEXT=$(agora-kiro inject --quiet)
    """
    db_path = str(tmp_path / "memory.db")
    env = {"AGORA_KIRO_DB": db_path}
...
def test_install_hooks_creates_skill_md(tmp_path, monkeypatch):
    """install-hooks --claude-code must create ~/.claude/skills/agora-kiro/SKILL.md (global only, not project-level)."""
    ...
    project_skill = tmp_path / ".claude" / "skills" / "agora-kiro" / "SKILL.md"
    assert not project_skill.exists(), "SKILL.md should not be installed at project level"
    # Global should be created
    skill_path = fake_home / ".claude" / "skills" / "agora-kiro" / "SKILL.md"
    assert skill_path.exists(), "SKILL.md was not created in ~/.claude/skills/"
    content = skill_path.read_text()
    assert "name: agora-kiro" in content, "SKILL.md missing frontmatter"
    assert "agora-kiro inject" in content, "SKILL.md missing inject command"
