...
from agora_kiro.memory_server import _dispatch, _TOOLS
...
def isolate_storage(monkeypatch, tmp_path):
    ...
    Strategy:
      ...
      - create .agora-kiro/ inside tmp_path so it finds a project root immediately
      - override AGORA_KIRO_DB env var and reset the store singleton
    """
    import agora_kiro.vector_store as vs_mod
    import agora_kiro.session as sess_mod

    # Create .agora-kiro dir so _find_project_root() anchors to tmp_path
    (tmp_path / ".agora-kiro").mkdir(parents=True, exist_ok=True)

    # chdir so Path.cwd() inside _find_project_root starts at tmp_path
    monkeypatch.chdir(tmp_path)

    # Redirect global session fallback so ~/.agora-kiro/session.json is not used
    monkeypatch.setattr(sess_mod, "_GLOBAL_DIR", tmp_path / ".agora-kiro-global")

    # Redirect vector store DB and reset singleton
    db_path = str(tmp_path / "test_memory.db")
    monkeypatch.setenv("AGORA_KIRO_DB", db_path)
