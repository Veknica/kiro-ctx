"""
test_session.py — Session lifecycle tests for agora_kiro/session.py.

All tests use tmp_path / monkeypatch so nothing writes to the real
~/.agora-kiro or .agora-kiro directories.
"""
...
def _patch_session_dir(monkeypatch, tmp_path: Path) -> Path:
    ...
    Creates .agora-kiro/ in tmp_path and chdir's there so _find_project_root
    ...
    # Redirect global fallback so ~/.agora-kiro/session.json is not used
...
def test_load_session_returns_none_on_corrupt_file(monkeypatch, tmp_path):
    _patch_session_dir(monkeypatch, tmp_path)
    session_dir = tmp_path / ".agora-kiro"
...
def test_load_if_recent_returns_none_when_stale(monkeypatch, tmp_path):
    ...
    session_file = tmp_path / ".agora-kiro" / "session.json"
...
def test_get_git_branch_in_actual_repo():
    """We ARE in a git repo (agora-kiro itself), so branch should not be None."""
    result = _get_git_branch()
    assert result is not None
    assert len(result) > 0
