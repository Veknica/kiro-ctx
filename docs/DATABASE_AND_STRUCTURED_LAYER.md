...
| **File structure summary (outline of a large file)** | **DB if cached** | Pre-read hook runs `agora-kiro summarize`. If we have a `file_snapshot` for that path at the **same git commit**, we serve it from DB and **do not read the file**. Otherwise we read from disk and summarize. |
...
|----------|--------|--------------|
| **Pre-read hook (large file)** | **Yes when cached** | `agora-kiro summarize` checks the DB first: if we have a `file_snapshot` for that file at the **same git commit**, we serve that summary and do **not** read from disk. Otherwise we summarize from disk and the on-read hook then writes to the DB. |
...
**Keeping the DB in sync:** On every file **edit**, the hooks call `index_file` (or `agora-kiro index`) so `file_snapshots` and `symbol_notes` are updated. **You do not need to commit** — we update the AST on every edit (we read the current file from disk and write to the DB). The stored `commit_sha` is current HEAD at index time; we do **not** store an "uncommitted" flag on snapshots or symbol_notes.
...
**When a commit is made:** The post-commit flow (e.g. `agora-kiro track-diff` after `git commit`, or Claude Code `on-bash.sh` when it detects a commit) calls `tag_commit()` → `tag_committed_files()`. That (1) marks **file_changes** rows for those paths as `status='committed'` and sets their `commit_sha` to the new commit (and leaves `recorded_at_commit_sha` unchanged), and (2) updates **symbol_notes** and **file_snapshots** for those paths so their `commit_sha` is set to the new commit. So after a commit, the DB reflects the new SHA and the summarize cache stays valid without re-reading the file.
...
| To see | Command |
|--------|---------|
| **DB path + counts + everything in one view** | `agora-kiro memory` or `agora-kiro memory 20` (sessions, learnings, snapshots, symbols). Add `--verbose` to see stored AST/code blocks. |
| **Sessions** | `agora-kiro list-sessions` or `agora-kiro restore` (no arg) |
| **Learnings** | `agora-kiro list-learnings` or `agora-kiro recall ""` |
| **File snapshots (AST)** | `agora-kiro list-snapshots` |
| **Symbol notes (functions/classes)** | `agora-kiro list-symbols` or `agora-kiro list-symbols --file path/to/file.py` |
| **File changes** | `agora-kiro list-file-changes` (recent across project) or `agora-kiro file-history <path>` (per file) |
| **API calls** | `agora-kiro list-api-calls` |
| **Current session + stats** | `agora-kiro status` |
...
- **On your machine**: The DB is at **`~/.agora-kiro/memory.db`** (or whatever `AGORA_KIRO_DB` is set to). To see the path and that it’s in use, run **`agora-kiro status`** — it prints the DB path and row counts. So the data is in that file; you don’t need SQL to “see” it — use the list-* commands above.
- **In a sandbox/CI**: If the process can’t write to `~/.agora-kiro/`, you get “unable to open database file”. Then set **`AGORA_KIRO_DB`** to a path you can write (e.g. `$(pwd)/.agora-kiro/memory.db`). That’s why the test used that; on your own machine you usually don’t need it.
...
agora-kiro memory
agora-kiro memory --verbose    # include stored AST/code block sample

# 3. Per-table view (no SQL)
agora-kiro list-sessions
agora-kiro list-learnings
agora-kiro list-snapshots
agora-kiro list-symbols
agora-kiro list-symbols --file agora_kiro/cli.py
agora-kiro list-file-changes
agora-kiro file-history agora_kiro/session.py
agora-kiro list-api-calls

# 4. Track diffs (single file or all uncommitted)
agora-kiro track-diff path/to/file.py
agora-kiro track-diff --all

# 5. Index a file → then summarize (should say "served from DB" when commit matches)
agora-kiro index agora_kiro/session.py
agora-kiro list-snapshots -n 3
...
- **Path**: `AGORA_KIRO_DB` env var, or default `~/.agora-kiro/memory.db`.
...
- **Home dir**: `Path.home() / ".agora-kiro"` for default DB and global session.
- **Temp**: `/tmp` only for hook temp files (e.g. `mktemp /tmp/agora_hook_XXXXXX`, `/tmp/agora-kiro-summaries`).
- **README/config**: Placeholders like `/full/path/from/which/agora-kiro/agora-kiro` for the user to replace.
