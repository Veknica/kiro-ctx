"""
session.py — Session lifecycle manager for agora-kiro.
...
File locations:
  - .agora-kiro/session.json   (project-local — take priority)
  - ~/.agora-kiro/session.json (global fallback)
...
_AGORA_DIR  = ".agora-kiro"
_SESSION_FILE = "session.json"
_GLOBAL_DIR = Path.home() / ".agora-kiro"


def _find_project_root(start: Optional[Path] = None) -> Optional[Path]:
    """
    Walk up from start until we find .agora-kiro/, .git/, or pyproject.toml.
    Returns None if nothing found (e.g. MCP server spawned from /). This
    signals callers to use the global ~/.agora-kiro fallback.
    """
...
def get_session_path(project_root: Optional[Path] = None) -> Path:
    ...
    Falls back to ~/.agora-kiro/session.json when no project root is found
...
def save_session(
    ...
    """
    Write session to .agora-kiro/session.json.
...
def _ensure_gitignore(agora_dir: Path) -> None:
    ...
    if not gi.exists():
        try:
            gi.write_text("# agora-kiro local state — do not commit\n*\n", encoding="utf-8")
        except Exception:
            pass
