...
_TOOLS = [
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "file_path": {"type": "string", "description": "Relative path to the file, e.g. 'agora_kiro/auth.py'"},
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "Path to the file, e.g. 'agora_kiro/cli.py'"
                }
            },
            "required": ["file_path"]
        }
...
async def _handle_get_session_context(params: dict) -> str:
    ...
    from agora_kiro.compress import session_restored_banner
...
async def _handle_save_checkpoint(params: dict) -> str:
    from agora_kiro.session import update_session
...
async def _handle_recall_learnings(params: dict, namespace: str = "personal") -> str:
    ...
    from agora_kiro.session import load_session, _get_git_branch, _get_uncommitted_files
...
async def _handle_get_memory_stats(params: dict) -> str:
    from agora_kiro.vector_store import get_store
    from agora_kiro.embeddings import is_available, provider_info
...
async def _handle_summarize_file(params: dict) -> str:
    from agora_kiro.summarizer import summarize_file, FILE_SUMMARY_TOKEN_BUDGET
    from agora_kiro.vector_store import get_store
    from agora_kiro.session import _get_project_id, _get_git_branch
...
async def serve_memory() -> None:
    ...
    try:
        from agora_kiro.session import load_session_if_recent, _get_git_branch
        from agora_kiro.compress import session_restored_banner
